...
class Circle_Class():
    def main_process(self):#this function is to create the window for the game and its distinct text and images.
        ...
        rect.undraw()
        img.undraw()
    def circle_position(self,win):#this assings the correct position for the circle it also helps it be centered due to the positons being integers
        punto=win.getMouse()
        x=int(punto.getX())
        y=int(punto.getY())
        points=0
        ...
        #Column 7
            ...
            points=5
        return(x,y,points) 
class Circle_management(Circle_Class):
    def winner(self,kpoint,cpoint,win):#verifies which of the two has the most points and assigns a winner
        if(kpoint>cpoint):
            rect=Rectangle(Point(50,120),Point(560,360))
            rect.setFill("black")
            rect.draw(win)
            ...
            Message6.undraw()
            rect.undraw()
    def position_verifier(self,xlist,ylist,x,y,win,puntos):#verifies if the positions picked by the user has been occupied in previous moves
        for i in range(len(xlist)):
            while(xlist[i]==x and ylist[i]==y):
                ...
                Message1.undraw()
                rect.undraw()
                x,y,puntos=self.circle_position(win)
        return(x,y,puntos)
...
def main():
    ...
    test=Circle_Class()#this is the first instance of the class Circle_Class()
    test.main_process()
    test1=Circle_management()#this is the first instance of the class Circle_management()
    for num in range(42):#this for loops itirates 42 times equaling to 42 moves 21 for MrKrabs and 21 for the Customer
        x,y,puntos=test.circle_position(test.win)
        x,y,puntos=test1.position_verifier(xlist,ylist,x,y,test.win,puntos)#x and y positions are sent to verify if these were previously selected and if so it propmts the user to pick again, the points are also re assinged with the new values of the positon that is picked.       
        xlist.append(x)
        ylist.append(y)
        ...
        if(num%2==0):#Krabs
            kpoint+=puntos
            circulo.setFill("Red")
            circulo.draw(test.win)   
        elif(num%2!=0):#Customer
            cpoint+=puntos
            circulo.setFill((color_rgb(75,83,32)))
            circulo.draw(test.win)
    test1.winner(kpoint,cpoint,test.win)#calls the winner class to determine who is the winner
